...
# Báo cáo hoàn thiện chức năng Điểm danh và Tính lương nhân viên

## 1. Kế hoạch triển khai tuần tự

1. Đọc và phân tích luồng hiện tại của Attendance và Payroll.
2. Chốt công thức tính lương chi tiết theo rule nghiệp vụ mới.
3. Thêm tính năng import CSV chấm công vào app và lưu xuống DB qua API hiện có.
4. Áp công thức mới vào trang Payroll để hiển thị rõ breakdown lương.
5. Test các lỗi cơ bản (lint/compile-level) trước khi báo cáo.
6. Tổng hợp kết quả vào file report.

Trạng thái: Hoàn thành toàn bộ 6/6 bước.

## 2. Đọc hiểu luồng và logic hiện tại

### 2.1 Attendance hiện tại

- Trang quản trị điểm danh đang lấy dữ liệu tháng từ API `/attendance`.
- Có tạo/cập nhật điểm danh thủ công qua `/attendance/manual` và `/attendance/:id`.
- Chưa có cơ chế import CSV chấm công hàng loạt.

### 2.2 Payroll hiện tại

- Trang lương đang đọc danh sách payroll đã generate từ backend theo kỳ lương.
- Nút Tính lương gọi API generate payroll của backend.
- UI có hiển thị tổng quỹ lương, nhưng công thức hiển thị còn đơn giản và chưa phản ánh đầy đủ các khoản cộng/trừ theo rule mới.

## 3. Công thức tính lương đã áp dụng

### 3.1 Quy ước chấm công

- 540 phút = 1 ngày công.
- Phần thời gian vượt 540 phút/ngày được tính overtime.
- Overtime quy đổi theo giờ: overtimeHours = overtimeMinutes / 60.

### 3.2 Công thức thực lãnh

Net Salary = (Base Salary / 26 x Work Days) + Allowance + Bonus + Overtime Hours x 100000 - Insurance - Tax - Late Deduction - Leave Deduction

Trong đó:

- Insurance = 5% x Base Salary
- Tax = 10% x Base Salary
- Late Deduction = số ngày trễ x 100000
- Leave Deduction = max(0, số ngày nghỉ - 3) x (Base Salary / 26)

### 3.3 Rule trạng thái dùng để tính phạt

- Trễ: `LATE`, `DELAY`
- Nghỉ để trừ vượt ngưỡng: `ABSENT`, `UNPAID_LEAVE`, `SICK_LEAVE`

## 4. Các thay đổi đã thực hiện

### 4.1 Import CSV chấm công

File: `src/pages/admin/AdminAttendancePage.jsx`

- Thêm nút Nhập CSV chấm công.
- Thêm parser CSV hỗ trợ dữ liệu có dấu nháy/phẩy.
- Mapping cột linh hoạt (`employee_id`, `work_date`, `check_in`, `check_out`, `status`, `note` và alias).
- Tự resolve nhân viên theo `employee_id`, hoặc fallback theo email/tên.
- Gửi từng dòng về API tạo điểm danh để lưu DB trước khi tính lương.
- Ghi nhận số dòng thành công/thất bại, hiển thị toast và log lỗi chi tiết.

### 4.2 Công thức lương và breakdown rõ ràng

File mới: `src/utils/payrollCalculator.js`

- Tạo module chuyên biệt chứa hằng số và hàm tính lương có chú thích rõ.
- Hàm chính: `calculatePayrollByAttendance(...)` trả về đầy đủ breakdown (ngày công, OT, thuế, bảo hiểm, trừ trễ, trừ nghỉ, thực lãnh).

### 4.3 Áp dụng công thức vào trang Payroll

File: `src/pages/admin/AdminPayrollPage.jsx`

- Bổ sung đọc attendance theo tháng để làm dữ liệu đầu vào cho công thức.
- Áp công thức lương mới cho từng nhân viên ở chế độ theo tháng.
- Nếu backend chưa generate payroll, vẫn hiển thị bảng tạm tính dựa trên dữ liệu chấm công.
- Mở rộng cột hiển thị: Ngày công, OT (giờ), Phụ cấp, Thưởng, Bảo hiểm, Thuế, Trừ trễ + nghỉ, Thực lãnh.
- Cập nhật phần mô tả công thức ngay trên UI để minh bạch cách tính.

## 5. Kiểm thử lỗi cơ bản

- Đã chạy eslint trực tiếp cho các file thay đổi:
...
- Build tổng thể chưa chạy vì thao tác build đã bị bỏ qua tại thời điểm thực thi lệnh.

## 6. Kết luận

- Đã hoàn thiện luồng nhập CSV chấm công vào app và lưu DB trước bước tính lương.
- Đã áp dụng cơ chế tính lương mới đúng công thức yêu cầu, có module riêng và chú thích rõ.
- Đã test lỗi cơ bản ở mức lint cho các file thay đổi trước khi báo cáo.
